...
From the table, we can see that the maximum profitability point is when Gain's price is decreased by 5% and Tide's price is increased by 5%. Of course, since this is the edge of our percentage delta range, it's possible that we can decrease Gain's price further and/or increase Tide's price further for more profits. However, it's probably prudent to start with smaller price changes and looking at the analysis in the future with new data, in case the current model does not predict well outside of 5% price changes.

This recommended price change is consistent with Tide and Gain's own-price elasticity estimates. Since Tide's own-price elasticity is smaller (absolute) than Gain's, it's better to increase Tide's price to and decrease Gain's price. This way, Tide's relative decrease in demand is more than offset by Gain's relative increase in demand. One note is that this assumption works because we assume that the profitability margins for Tide and Gain are identical.