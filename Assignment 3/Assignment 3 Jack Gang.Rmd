...
We estimated 4 different models and clustered standard errors at the market/year level for each. First, the base model used the main variables, `log(income)`, the unemployed indicator, and the wealth measure (`log(zillow_index)`), as covariates in the regression. The dependent variable was the household-level private label share in each month. In this base model we pooled across all units in the panel:
